...
class GameManager(models.Manager):
  ...
  def new_game(self,
    ...
    game_id = self.new_id()
    game    = self.create(game_id       = game_id,
    ...
    initial_state = Game(game_id = game_id)
    initial_state.save()
...
class Game(models.Model):
  ...
  def get_history(self):
    ...
    history = (Board.objects
               .filter(game_id == self.game_id)
               .annotate(
                  move_recency = Window(
                    expression  = RowNumber(),
                    partitionBy = F('move_num'),
                    orderBy     = F('date_created').desc()
                ))
               .filter(move_recency == 1)
               .orderBy(date_created.asc())
    )

    latest_move_num = history.last().move_num

    history = (history
               .filter(move_num__lte = latest_move_num)
               .orderBy(move_num.asc())
    )
